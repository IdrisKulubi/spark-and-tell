...
export function SparkAwarder({ onAward, recipientName, basePointsAwarded }: ExtendedSparkAwarderProps) {
    const [selectedSparks, setSelectedSparks] = useState<SparkType[]>([]);
    const [showConfetti, setShowConfetti] = useState(false);

    const toggleSparkSelection = (sparkType: SparkType) => {
        setSelectedSparks((prev) => (prev.includes(sparkType) ? prev.filter((s) => s !== sparkType) : [...prev, sparkType]));
    };
    ...
    return (
        <div className="mx-auto w-full max-w-2xl">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-6 text-center">
                <h3 className="mb-2 font-bold text-xl sm:text-2xl text-white">Award Sparks to {recipientName}! ✨</h3>
                {basePointsAwarded && (
                    <p className="mb-1 text-sm text-white/70">
                        ({recipientName} already earned {basePointsAwarded} base points)
                    </p>
                )}
                <p className="text-sm sm:text-base text-white/80">How did their answer make you feel?</p>
            </motion.div>
                ...
                {SPARK_REACTIONS.map((reaction, index) => (
                    <motion.button
                        ...
                        className={`relative rounded-xl sm:rounded-2xl p-3 sm:p-4 transition-all ${
                            selectedSparks.includes(reaction.type) ? "scale-105 bg-white shadow-xl" : "bg-white/20 hover:bg-white/30"
                        }`}
                    >
                        <div className="mb-1 text-2xl sm:text-3xl">{reaction.emoji}</div>
                        <div
                            className={`font-semibold text-[10px] sm:text-xs ${
                                selectedSparks.includes(reaction.type) ? "text-purple-600" : "text-white"
                            }`}
                        >
                            {reaction.label}
                        </div>
            ...
            {selectedSparks.length > 0 && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="mb-4 text-center">
                    <div className="text-base sm:text-lg text-white">
                        Total: <span className="font-bold text-xl sm:text-2xl">+{totalPoints}</span> sparks
                    </div>
                </motion.div>
            )}
            ...
            <motion.button
                ...
                className="w-full rounded-full bg-white py-3 sm:py-4 font-bold text-lg sm:text-xl text-purple-600 shadow-xl transition-all hover:shadow-white/30"
            >
                {selectedSparks.length > 0 ? `Award ${totalPoints} Sparks! 🎉` : "Skip - No Sparks"}
            </motion.button>
