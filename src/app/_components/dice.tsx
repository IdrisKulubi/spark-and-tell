	...
	return (
		<div className="flex flex-col items-center space-y-4 sm:space-y-6">
			...
			<motion.div
				...
				className="grid grid-cols-2 gap-2 overflow-hidden sm:gap-3 md:grid-cols-3"
			>
					...
					return (
						<motion.div
							...
							animate={{
								...
								opacity: showLegend
									? highlightedCategory && !isHighlighted
										? 0.3
										: 1
									: 0,
							}}
							...
							} px-2 py-1.5 font-semibold text-white text-xs shadow-lg sm:px-3 sm:py-2 sm:text-sm`}
						>
			...
			{!isRolling && !disabled && !highlightedCategory && (
				...
				>
					...
					<p className="mt-1 text-white/70 text-xs sm:text-sm">
						Each number is a different category
					</p>
				</motion.div>
			)}
			...
			{isRolling && (
				<motion.div
					initial={{ opacity: 0 }}
					animate={{ opacity: 1 }}
					className="font-semibold text-base text-white sm:text-lg"
				>
			...
			{highlightedCategory && !isRolling && (
				...
				>
					<p className="font-bold text-white text-xl sm:text-2xl">
						You rolled: {CategoryNames[highlightedCategory]}!
					</p>
					<p className="mt-1 text-white/70 text-xs sm:text-sm">
